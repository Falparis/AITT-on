...
describe('frameworks read-only', () => {
  it('is read-only (no direct create endpoint)', async () => {
    await runSeed();
    const list = await frameworkService.listFrameworks({});
    // Regulations (GDPR/ISO/…) are no longer seeded — the platform stopped
    // managing them. The endpoint stays read-only regardless.
    expect(list.pagination.total).toBeGreaterThanOrEqual(0);
...
describe('seed-p5', () => {
  it('is idempotent', async () => {
    ...
    // Frameworks/regulations are no longer seeded; programs + templates are.
    expect(first.programs).toBeGreaterThan(0);
    const second = await runSeed();
    expect(second.frameworks).toBe(0);
    expect(second.templates).toBe(0); // already seeded
