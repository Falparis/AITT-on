...
function toDocItem(cert) {
  ...
  return {
    ...
    hash: c.metadataHash,
    // Whether a downloadable file is actually stored. Not every workflow has
    // one — a certification case can be created without an upload (supporting
    // docs live off-platform) — so the UI needs this to avoid offering a
    // Download button that can only 404.
    hasFile: !!(c.storage && (c.storage.key || c.storage.path)),
    ...(txHash ? { txHash } : {}),
    ...
    // Workflow reframe: two-value status (a certification case is "completed"
    // once it reaches the registry) + the assigned reviewer.
    workflowStatus: ['issued', 'revoked', 'expired'].includes(c.status) ? 'completed' : 'in_progress',
    ...(c.assignedReviewerName ? { assignedReviewer: c.assignedReviewerName } : {}),
    ...(c.assignedReviewerId ? { assignedReviewerId: String(c.assignedReviewerId._id || c.assignedReviewerId) } : {}),
