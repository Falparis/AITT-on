...
async function activateSubAdmin(id, { adminUserId = null, adapter = getAdapter() } = {}) {
  const sa = await SubAdmin.findById(id).select('+walletSecretEnc');
  ...
  // Option 2: with SPONSOR_FEES on, the secret lets us sponsor-create (0 balance).
  await fundIfEnabled(sa.walletAddress, sa.walletSecretEnc);
