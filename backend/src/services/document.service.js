 ...
 */
async function submitDocument({ buffer, filename, subject, programId = null, assignedReviewerId = null, mimeType = null, size = null, companyId, requestedByUserId, adapter = getAdapter() }) {
  ...
  // Resolve the assigned reviewer: a Sub-Admin id, or the 'main_admin' sentinel.
  let reviewerId = null;
  let reviewerName = null;
  if (assignedReviewerId) {
    if (assignedReviewerId === 'main_admin') {
      reviewerName = 'Main Admin';
    } else {
      const sa = await SubAdmin.findById(assignedReviewerId);
      if (!sa) throw new AppError(404, 'Assigned reviewer not found');
      reviewerId = sa._id;
      reviewerName = sa.name;
    }
  }
  ...
  if (company.status !== 'active') {
    throw new AppError(403, 'Company must be approved (whitelisted) before creating a workflow');
  }

  // Snapshot the program + assigned reviewer onto a certificate/workflow record.
  const applyMeta = (cert) => {
    if (program) {
      cert.programId = program._id;
      cert.programName = program.name;
      cert.programType = program.type;
      cert.jurisdiction = program.jurisdiction;
    }
    if (assignedReviewerId) {
      cert.assignedReviewerId = reviewerId;
      cert.assignedReviewerName = reviewerName;
    }
  };

  // No-file workflow (current model — supporting docs live off-platform, e.g.
  // Airtable). The certification case is created in the DB only; the Final
  // Certificate PDF is uploaded + anchored on-chain later, at issue time.
  if (!buffer || !buffer.length) {
    const wfHash = sha256(crypto.randomBytes(32));
    const cert = await indexer.mirrorStoredDocument({
      metadataHash: wfHash, certificateName: filename, subject: resolvedSubject, companyId, requestedByUserId,
      network: company?.metadata?.network || 'testnet',
    });
    applyMeta(cert);
    await cert.save();
    logger.info('Workflow created (no file, off-chain)', { id: cert._id, companyId: String(companyId) });
    return asDocItem(cert, company.name);
  }
...
async function listDocuments({ user, page = 1, limit = 20, status = null, workflowStatus = null }) {
  ...
  // Workflow status filter: completed = reached the registry; in_progress = rest.
  const COMPLETED = ['issued', 'revoked', 'expired'];
  if (workflowStatus === 'completed') filter.status = { $in: COMPLETED };
  else if (workflowStatus === 'in_progress') filter.status = { $nin: COMPLETED };
  // Only accept a known string status (audit #8: reject injected objects).
  else if (typeof status === 'string' && DOC_STATUSES.includes(status)) filter.status = status;
 ...
 */
async function issueDocument({ id, issuerUserId, expiryAt = null, buffer = null, filename = null, mimeType = null, adapter = getAdapter() }) {
  const cert = await Certificate.findById(id).populate('companyId', 'name');
  if (!cert) throw new AppError(404, 'Document not found');
  ...
  // Final Certificate PDF (the only file the platform stores). For a workflow
  // that was never anchored (created without a file), the PDF's hash becomes the
  // certificate's verifiable on-chain hash so public verify-by-file works.
  //
  // The contract's issue_certificate REQUIRES that hash to ALREADY exist on-chain
  // as a Submitted document, so the PDF must be anchored with store_document
  // first. Nothing did that: submitDocument's no-file branch promises the anchor
  // happens "later, at issue time", and issue went straight to issue_certificate.
  // Every workflow created without a file therefore failed at issue with an
  // opaque chain error — Error(WasmVm, InvalidAction) on the real adapter,
  // "document not found" on the stub. Two symptoms, this one gap.
  //
  // Anchored as the MAIN ADMIN rather than the company: the Final Certificate is
  // the platform's own artifact, main admin already signs the issue call that
  // follows, and a company custodial wallet may not exist on-chain yet (it is
  // created and whitelisted at approval time) — which would fail here for a
  // reason that has nothing to do with issuing.
  if (buffer && buffer.length && !cert.chain?.txHashStore) {
    const pdfHash = sha256(buffer);
    const clash = await Certificate.findOne({ metadataHash: pdfHash, _id: { $ne: cert._id } });
    if (clash) throw new AppError(409, 'A certificate with the same file hash already exists');
    cert.metadataHash = pdfHash;
    await cert.save();

    const certName = filename || cert.certificateName || cert.originalFilename || String(cert._id);
    await indexer.writeThrough({
      adapter,
      method: 'storeDocument',
      args: [mainAdmin, certName, pdfHash, {}],
      purpose: 'store',
      meta: { certificateId: cert._id, submittedByUserId: issuerUserId },
      mirror: {
        op: 'mirrorStoredDocument',
        payload: {
          metadataHash: pdfHash,
          certificateName: certName,
          subject: cert.subject,
          companyId: cert.companyId?._id || cert.companyId,
          requestedByUserId: cert.requestedByUserId,
          network: cert.chain?.network || 'testnet',
        },
      },
    });
    logger.info('Final Certificate PDF anchored at issue time', {
      id: cert._id,
      hashShort: pdfHash.slice(0, 12),
    });
  }
  const { mirrored } = await indexer.writeThrough({
    adapter,
  ...
  // Persist the Final Certificate PDF so it can be downloaded from the registry.
  if (buffer && buffer.length) {
    try {
      const stored = await storage.saveBuffer(buffer, {
        filename: filename || 'certificate.pdf',
        mimeType: mimeType || 'application/pdf',
      });
      mirrored.storage = { provider: stored.provider, key: stored.key, path: stored.path || undefined };
      mirrored.originalFilename = filename || 'certificate.pdf';
      mirrored.mimeType = mimeType || 'application/pdf';
      await mirrored.save();
    } catch (e) {
      logger.warn('Final certificate PDF storage failed (certificate remains issued)', { id: mirrored._id, error: e.message });
    }
  }
