...
async function submitDocument(req, res, next) {
  try {
    const { subject, programId, assignedReviewerId } = req.body || {};
    const filename = req.body.filename || req.file?.originalname || 'Certification workflow';
    // File is now OPTIONAL — workflows are created without company documents
    // (supporting docs live off-platform). A program or subject is still required.
    if (!subject && !programId) return res.status(400).json({ success: false, message: 'subject or programId is required' });

    // Company admins create for their OWN company; admins may pass companyId.
...
async function listDocuments(req, res, next) {
  try {
    const { page = 1, limit = 20, status, workflowStatus } = req.query;
    const result = await documentService.listDocuments({ user: req.user, page, limit, status, workflowStatus });
...
async function issueDocument(req, res, next) {
  try {
    const doc = await documentService.issueDocument({
      ...
      // Optional Final Certificate PDF (multipart) — the only file we store.
      buffer: bufferFromReq(req),
