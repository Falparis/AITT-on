...
const sponsorFeesEnabled = () => String(process.env.SPONSOR_FEES || '').toLowerCase() === 'true';
 ...
 */
async function fundIfEnabled(publicKey, walletSecretEnc = null) {
  if (selectedAdapter() !== 'real') return { funded: false, skipped: 'stub-mode' };

  // Option 2 — when fee-sponsorship is on, CREATE the account from the central
  // (service) account with sponsored reserves so the user holds ZERO XLM; its
  // fees are then covered by fee-bumps. This runs INDEPENDENTLY of the legacy
  // AUTO_FUND_WALLETS flag — the account must exist even though it holds no XLM.
  if (sponsorFeesEnabled() && walletSecretEnc) {
    try {
      const kp = Keypair.fromSecret(decryptSecret(walletSecretEnc));
      const res = await rpc.sponsorCreateAccount(kp);
      return { funded: true, sponsored: true, ...res };
    } catch (err) {
      logger.warn('sponsored creation failed (non-fatal — retry via ops)', { pub: publicKey && publicKey.slice(0, 8), error: err.message });
      return { funded: false, error: err.message };
    }
  }

  // Legacy friendbot funding (testnet), gated by AUTO_FUND_WALLETS.
