...
function getConfig() {
  ...
  _config = {
    ...
    // Option 2 — the central (service) account pays EVERY user's fee via a
    // fee-bump, and sponsors new custodial accounts so users hold zero XLM.
    // Opt-in so the current per-wallet flow is untouched until enabled.
    sponsorFees: String(process.env.SPONSOR_FEES || '').toLowerCase() === 'true',
  };
  return _config;
}
...
async function _sendTxInner(method, args, kp) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      // Option 2 — the central (service) account pays the fee via a fee-bump,
      // unless the signer already IS the service account (it pays as source).
      // The fee-bump wraps the already-prepared+signed inner tx (its per-op fee
      // must be >= the inner Soroban fee, which for a 1-op invocation = tx.fee).
      let submitTx = tx;
      if (cfg.sponsorFees && kp.publicKey() !== serviceKP.publicKey()) {
        ...
        submitTx.sign(serviceKP);
        logger.info('fee-bump applied', { method, feePayer: serviceKP.publicKey().slice(0, 8), signer: kp.publicKey().slice(0, 8) });
      }

      const sendResp = await server.sendTransaction(submitTx);
...
/**
 * sponsorCreateAccount(userKP) — Option 2 onboarding: the central (service)
 * account CREATES a new custodial account with 0 starting balance and SPONSORS
 * its base reserve, so the user holds zero XLM yet can still transact (its fees
 * are covered by fee-bumps). Signed by BOTH the sponsor (service) and the
 * sponsored account (userKP, which authorizes end-sponsoring). Idempotent: if
 * the account already exists it is a no-op.
 */
async function sponsorCreateAccount(userKP) {
  const cfg = getConfig();
  const { server, serviceKP } = getClients();
  const dest = userKP.publicKey();

  try {
    await server.getAccount(dest);
    return { created: false, alreadyExists: true };
  } catch (_) {
    // not found → create it below
  }

  return runExclusive(`tx:${serviceKP.publicKey()}`, async () => {
    const src = await server.getAccount(serviceKP.publicKey());
    const tx = new TransactionBuilder(src, { fee: BASE_FEE, networkPassphrase: cfg.NETWORK_PASSPHRASE })
      .addOperation(Operation.beginSponsoringFutureReserves({ sponsoredId: dest }))
      .addOperation(Operation.createAccount({ destination: dest, startingBalance: '0' }))
      .addOperation(Operation.endSponsoringFutureReserves({ source: dest }))
      .setTimeout(60)
      .build();
    tx.sign(serviceKP, userKP); // sponsor + sponsored both sign
    const resp = await server.sendTransaction(tx);
    if (resp.errorResult) {
      throw new AppError(502, 'sponsored account creation failed', txErrorName(resp.errorResult) || 'submit error');
    }
    await waitForTransaction(server, resp.hash, {
      timeoutMs: cfg.txTimeoutMs, pollIntervalMs: cfg.pollIntervalMs, pollMaxIntervalMs: cfg.pollMaxIntervalMs,
    });
    logger.info('sponsored custodial account created (0 balance)', { pub: dest.slice(0, 8), hash: resp.hash });
    return { created: true, hash: resp.hash };
  });
}
