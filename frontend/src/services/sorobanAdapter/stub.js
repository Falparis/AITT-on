...
// A stub "document not found" is almost never a real missing document — the DB
// lookup upstream already succeeded, so what actually happened is that this
// process's in-memory chain state was reset (deploy / crash / restart) and the
// earlier store_document went with it. Say so, because the bare message is
// indistinguishable from the DB's own 404 and sends people hunting in the wrong
// place. See docs/API.md "Chain adapter".
const STUB_DOC_MISSING =
  'document not found on the in-memory stub chain. The stub keeps all chain ' +
  'state in RAM, so anything stored before the last restart is gone. If this is ' +
  'a real deployment, set SOROBAN_ADAPTER=real (plus RPC_URL / CONTRACT_ID / ' +
  'SERVICE_SECRET) so documents are anchored on the actual contract.';
  ...
  function executeAction(action) {
    switch (action.type) {
      case 'RevokeCertificate': {
        ...
        if (!doc) throw new AppError(404, STUB_DOC_MISSING);
        doc.status = 'Revoked';
        break;
      }
