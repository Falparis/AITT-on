...
const { runTreasuryBalanceJob } = require('./jobs/treasuryBalance.job');
...
function startScheduler() {
  ...
  const treasuryMs = Number(process.env.TREASURY_JOB_INTERVAL_MS || 1800000); // 30 min
  ...
  // Central fee-account low-balance monitor (Option 2).
  registerInterval('treasury', treasuryMs, 5 * 60 * 1000, () => runTreasuryBalanceJob({}));

  logger.info('Scheduler started', { outboxMs, expiryMs, reconcileMs, treasuryMs });
}
