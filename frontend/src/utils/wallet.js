...
function assertKeyOrPlaintextAllowed(key) {
  ...
  const allowPlain = env === 'development' || String(process.env.ALLOW_PLAINTEXT_KEYS).toLowerCase() === 'true';
  if (!allowPlain) {
...
//
// These throw AppError, not bare Error, on purpose: a bare Error reaches the
// client as an opaque "Internal Server Error" with the real cause buried in the
// server log, and this particular failure is a CONFIG problem (missing or
// changed KEY_ENCRYPTION_SECRET) that an operator can fix in seconds once they
// can see it. It costs hours when it looks like a generic crash.
function decryptGcm(stored, key) {
  if (!key) {
    throw new AppError(
      500,
      'Cannot decrypt the custodial wallet: KEY_ENCRYPTION_SECRET is not set on this server. ' +
        'It must be the SAME value used when the wallet was created — a different one will not work.'
    );
  }
  ...
  try {
    ...
    // GCM auth-tag failure is what a WRONG key looks like — indistinguishable
    // from corruption at this layer, but in practice it is always the key.
    throw new AppError(
      500,
      'Cannot decrypt the custodial wallet: the KEY_ENCRYPTION_SECRET on this server does not ' +
        'match the one this wallet was encrypted with.',
      err.message
    );
  }
}
