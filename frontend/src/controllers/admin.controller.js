...
const { runTreasuryBalanceJob } = require('../services/jobs/treasuryBalance.job');
...
// POST /admin/jobs/treasury-balance — check the central fee account's balance
// now and alert/notify admins if it is low (Option 2 monitor).
async function runTreasuryBalance(req, res, next) {
  try {
    const warnBelow = req.body?.warnBelow != null ? Number(req.body.warnBelow) : undefined;
    const critBelow = req.body?.critBelow != null ? Number(req.body.critBelow) : undefined;
    const result = await runTreasuryBalanceJob({ warnBelow, critBelow });
    logger.info('Treasury-balance check triggered via API', { by: req.user.sub, result });
    res.json({ success: true, data: result });
  } catch (err) {
    next(err);
  }
...
module.exports = { runExpiry, runOutbox, runReconcile, runTreasuryBalance };
